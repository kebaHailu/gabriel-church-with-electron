...
export default function Frewoch() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting, event }) => {
      if (
        !formik.values.date &&
        !formik.values.sex &&
        !formik.values.marriage &&
        !formik.values.churchContrbution &&
        !formik.values.cheducation &&
        !formik.values.otherchurch &&
  ...
  return (
    <div className="title">
      <Typography variant="h4">የፍሬ አባላት ቅጽ መሙያ</Typography>
      ...
      <form id="wetatoch-form" onSubmit={formik.handleSubmit}>
        <div className="personal-info">
          <Box sx={{ "& .MuiTextField-root": { m: 1, width: "30ch" } }}>
            ...
            <ProfilePictureUploader onImageSelected={handleImageSelected} />
